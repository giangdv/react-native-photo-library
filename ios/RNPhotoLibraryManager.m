...
#import "RNPhotoLibraryManager.h"
...
#import "RNAssetsLibraryRequestHandler.h"
...
@implementation RNPhotoLibraryManager

RCT_EXPORT_MODULE(RNPhotoLibrary)
                  ...
                  reject:(RCTPromiseRejectBlock)reject)
{
    ...
    assetFetchOptions.sortDescriptors = @[[NSSortDescriptor sortDescriptorWithKey:@"creationDate" ascending:orderByAsc]];
    
    if (beginCreated > 0) {
        NSPredicate* predicate = [NSPredicate predicateWithFormat:@"creationDate > %@", [NSDate dateWithTimeIntervalSince1970: beginCreated]];
        if (assetFetchOptions.predicate != nil) {
            predicate = [NSCompoundPredicate andPredicateWithSubpredicates:@[assetFetchOptions.predicate, predicate]];
        }
        assetFetchOptions.predicate = predicate;
    }
    
    if (endCreated > 0) {
        NSPredicate* predicate = [NSPredicate predicateWithFormat:@"creationDate < %@", [NSDate dateWithTimeIntervalSince1970: endCreated]];
        if (assetFetchOptions.predicate != nil) {
            predicate = [NSCompoundPredicate andPredicateWithSubpredicates:@[assetFetchOptions.predicate, predicate]];
        }
        assetFetchOptions.predicate = predicate;
